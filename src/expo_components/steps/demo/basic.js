			...
			<View style={{ marginTop: 60 }}>
				<WingBlank size="lg">
					<Steps size="small" current={1}>
						{this.state.steps1.map((item, index) => (<Step key={index} title={<View><Text>title:{item.title}</Text></View>} description={<View><Text>desc:{item.description}</Text></View>} status={item.status}/>))}
					</Steps>
				</WingBlank>
			</View>
			<View>
			...
			<View>
				<WingBlank size="lg">
					<Steps current={1}>
						...
						<Step key={2} title="Wait" description="This is description" status="wait" icon={<Icon type="down" size={20} color="white"/>}/>
					</Steps>
				</WingBlank>
			</View>
		</ScrollView>);

...
export default class BasicDemo extends React.Component {
	constructor(props) {
		...
		this.renderSectionHeader = (sectionData) => {
			return (<Text style={[{ padding: 10, backgroundColor: 'rgba(255,255,255,0.8)' }]}>
	...
	render() {
		...
		return (<ListView dataSource={this.state.dataSource} renderHeader={() => <Text style={[{ padding: 8 }]}>header</Text>} renderFooter={() => <Text style={[{ padding: 30, textAlign: 'center' }]}> {loadingTxt} </Text>} renderSectionHeader={this.renderSectionHeader} renderRow={row} renderSeparator={separator} pageSize={4} onEndReached={this.onEndReached} onEndReachedThreshold={10}/>);
	}
}
